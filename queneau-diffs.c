...
short* queneau_sieve(const long chunk_start, long* chunk_count)
{
    int num_primes;
    long i, j;
    short diff;
    ...
    *chunk_count = root_sieve(chunk_start, num_primes, skip);
    ...
    qlist = (short*) malloc((*chunk_count + 1) * sizeof(short));
    ...
    for(j = 0; j < chunk_size; j++)
    {
        //if(diff == 65535) printf("Difference Overflow: %li", chunk_size + j);
        diff++;
    ...
    //Last entry is difference to next chunk_start
    qlist[i] = diff + 1;
...
int main(int argc, char* argv[])
{
    ...
    short** lists = (short**) malloc(nchunks * sizeof(short*));
    ...
    //Write start value to output file
    fwrite(&start, sizeof(long), 1, o_file);
    ...
    for(int i = 0; i < nchunks; i++)
    {
        ...
        //Add previous last difference
        lists[i][0] += last_diff;
        
        //Write Queneau numbers to output file
        for(j = 0; j < counts[i]; j++)
            //fprintf(o_file, "%hi\n", lists[i][j]);
            fwrite(&lists[i][j], sizeof(short), 1, o_file);

        //Last value is difference to next chunk_start
        last_diff = lists[i][j];
        
        //Delete each chunk qlist after writing
        free(lists[i]);
    }
    //Write difference to next block start at the very end
    fwrite(&last_diff, sizeof(short), 1, o_file);
    fclose(o_file);
